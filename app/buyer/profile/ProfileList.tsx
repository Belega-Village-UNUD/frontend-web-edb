...
const ProfileList = () => {
  ...
  const handleGetProfile = useCallback(async () => {
    try {
      ...
      if (responseJson.success === true) {
        ...
        setDefaultProvince({
          label: responseJson.data.profile.city.province,
          value: responseJson.data.profile.city.province_id,
        });
        setDefaultCity({
          label: responseJson.data.profile.city.city_name,
          value: responseJson.data.profile.city.city_id,
        });
        setProvince(responseJson.data.profile.city.province_id);
        setCity(responseJson.data.profile.city.city_id);
  ...
  const handleChangePassword: SubmitHandler<FieldValues> = async (data) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/auth/password/change`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="space-y-10 divide-y divide-gray-900/10 m-3 mb-10">
      ...
      <div className="grid grid-cols-1 gap-x-8 gap-y-8 pt-10 md:grid-cols-3">
        ...
        <form className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl md:col-span-2">
          ...
          <div className="flex items-center justify-end gap-x-3 border-t border-gray-900/10 px-4 py-8 sm:px-8">
            <ButtonConfirm
              outline
              label={isLoading ? "" : "Change Password"}
              loading={isLoading}
              onClick={setModalPassword}
            />
          </div>
        </form>
      </div>
                      ...
                      <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        ...
                        <div className="mt-2">
                          <p className="text-sm text-gray-500">
                            Are you sure you want to change password your
                            account?
                          </p>
                        </div>
                      </div>
